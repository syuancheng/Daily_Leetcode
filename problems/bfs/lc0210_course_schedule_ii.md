...
Return one valid order to finish all courses, or an empty array if the
prerequisite graph contains a cycle.

## My Solution

Build a directed graph where `from -> to` means taking `from` unlocks `to`.
Use DFS with two states:

- `visited[u]`: course `u` has already been fully processed, so later DFS calls
  can skip it.
- `onPath[u]`: course `u` is on the current recursion stack. Seeing it again
  means the graph has a cycle.

When a node finishes, append it to `postorder`. Reversing postorder gives a
